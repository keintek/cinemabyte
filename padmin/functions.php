...
function obtener_usuario($conexion, $user){
	$resultado = $conexion->query("SELECT * FROM usuarios WHERE cuenta = '$user' LIMIT 1");
...
function obtener_cafcombos($conexion){
	$sentencia = $conexion->prepare("SELECT * FROM cafeteria_combo;");
	$sentencia->execute();
	return $sentencia->fetchAll();
}
function obtener_cafproducts($conexion){
	$sentencia = $conexion->prepare("SELECT * FROM cafeteria_producto;");
	$sentencia->execute();
	return $sentencia->fetchAll();
}
